    ...
    return messages.map((msg, index) => (
      <div key={index} className={`flex mb-4 ${msg.position === 'right' ? 'justify-end' : ''}`}>
        <div className="flex items-center">
          {msg.position === 'left' && (
            <img
              src={AiAvatar} // Use bot avatar
              alt="Assistant"
              className="w-10 h-10 rounded-full mr-3 hidden md:block"
              style={{ alignSelf: 'flex-start' }}
            />
          )}
          <div className={`flex flex-col ${msg.position === 'right' ? 'items-end' : 'items-start'}`}>
            <div className={`text-xs font-semibold mb-1 ${msg.position === 'right' ? 'text-right' : 'text-left'}`}>
              {msg.senderName}
            </div>
            <div className={`p-3 rounded-lg max-w-xs ${msg.position === 'right' ? 'bg-[#1346AC] text-white' : 'bg-gray-300 text-black'}`}>
              <div>{msg.text}</div>
            </div>
          ...
          {msg.position === 'right' && residentData?.profilepic && (
            <img
              src={residentData.profilepic} // Use user's profilepic
              alt="User"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 p-8 bg-gray-100">
          <div className="h-full w-full bg-white rounded-lg shadow-md relative flex border border-gray-300 max-h-full">
            ...
            {/* Chat Window */}
            <div className="w-3/4 flex flex-col border-l border-gray-300">
